...
class ExportMaterialSamplesJob extends JobBase {
    static String trigger = 'exportMatSamplesJobCronTrigger'
    static String group = 'mnroad'
    static Logger log = Logger.getLogger(ExportMaterialSamplesJob.class)
    ...
    def execute(JobExecutionContext context) {
      ...
      try {
        ...
        res = exportDataService.dpMaterials(context.mergedJobDataMap)
      }
      catch (Exception e) {
        println "ExportMatSamplesJob.execute exception: ${e.message}"
        res = "Abend: ${e.message}"
        if (jr.id) {
          jr.result = res
          jr.save(flush:true)
        } else {
          println "Unable to store result for job ExportMatSamples"
        }
      }
      def ed = new Date()
      if (!jr.id) {
