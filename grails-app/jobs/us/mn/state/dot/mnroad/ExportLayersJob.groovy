...
class ExportLayersJob extends JobBase {
    static String trigger = 'exportLayersJobCronTrigger'
    static String group = 'mnroad'
    static Logger log = Logger.getLogger(ExportLayersJob.class)
    ...
    def execute(JobExecutionContext context) {
      ...
      try {
        ...
        res = exportDataService.dpLayers(context.mergedJobDataMap)
      }
      catch (Exception e) {
        println "ExportLayersJob.execute exception: ${e.message}"
        res = "Abend: ${e.message}"
        if (jr.id) {
          jr.result = res
          jr.save(flush:true)
        } else {
          println "Unable to store result for job ExportLayers"
        }
      }
      def ed = new Date()
      if (!jr.id) {
