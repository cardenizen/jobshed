...
class ExportDcpJob extends JobBase {
    static String trigger = 'exportDcpJobCronTrigger'
    static String group = 'mnroad'
    static Logger log = Logger.getLogger(ExportDcpJob.class)
    ...
    def execute(JobExecutionContext context) {
      ...
      try {
        ...
        res = exportDataService.dpDcp(context.mergedJobDataMap)
      }
      catch (Exception e) {
        println "ExportDcpJob.execute exception: ${e.message}"
        res = "Abend: ${e.message}"
        if (jr.id) {
          jr.result = res
          jr.save(flush:true)
        } else {
          println "Unable to store result for job ExportDcp"
        }
      }
      def ed = new Date()
      if (!jr.id) {
