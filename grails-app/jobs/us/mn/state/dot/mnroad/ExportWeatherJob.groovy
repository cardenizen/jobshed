 ...
 */
class ExportWeatherJob extends JobBase implements InterruptableJob {
  static String trigger = 'exportWeatherJobCronTrigger'
  ...
  void execute(JobExecutionContext context) {
    ...
    try {
      ...
      res = exportDataService.dpWeather(context.mergedJobDataMap)
    }
    catch (Exception e) {
      println "ExportWeatherJob.execute exception: ${e.message}"
      res = "Abend: ${e.message}"
      if (jr.id) {
        jr.result = res
        jr.save(flush:true)
