...
class ExportDistressJob extends JobBase {
    static String trigger = 'exportDistressJobCronTrigger'
    static String group = 'mnroad'
    static Logger log = Logger.getLogger(ExportDistressJob.class)
    ...
    def execute(JobExecutionContext context) {
      ...
      try {
        ...
        res = exportDataService.dpDistress(context.mergedJobDataMap)
      }
      catch (Exception e) {
        println "ExportDistressJob.execute exception: ${e.message}"
        res = "Abend: ${e.message}"
        if (jr.id) {
          jr.result = res
          jr.save(flush:true)
        } else {
          println "Unable to store result for job ExportDistress"
        }
      }
      def ed = new Date()
      if (!jr.id) {
